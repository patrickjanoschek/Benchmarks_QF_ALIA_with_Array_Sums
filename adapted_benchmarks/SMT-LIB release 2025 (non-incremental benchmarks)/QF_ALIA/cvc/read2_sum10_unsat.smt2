...
(assert (sum S_prime (+ v2 (+ a_prime y))))
(assert (sum bS (+ x (+ y bz))))
(assert (sum S (+ bv (+ v1 a2))))
(assert (sum bS (+ zz (+ v_prime v2))))
(assert (sum bS (+ a (+ v a_prime))))
(assert (sum S1 (+ zz (+ v a_prime))))
(assert (sum bS (+ v2 (+ ba ba))))
(check-sat)
(exit)