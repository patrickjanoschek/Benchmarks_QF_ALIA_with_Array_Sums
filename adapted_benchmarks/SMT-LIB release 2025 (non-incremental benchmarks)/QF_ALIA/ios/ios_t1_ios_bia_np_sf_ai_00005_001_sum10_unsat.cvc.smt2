(set-info :smt-lib-version 2.6)
(set-logic QF_ALIA)
(set-info :notes "Adapted for Array Sum evaluation.
Base file: ios_t1_ios_bia_np_sf_ai_00005_001.cvc.smt2")
(set-info :source |
Benchmarks used in the followin paper:
Big proof engines as little proof engines: new results on rewrite-based satisfiability procedure
Alessandro Armando, Maria Paola Bonacina, Silvio Ranise, Stephan Schulz. 
PDPAR'05
http://www.ai.dist.unige.it/pdpar05/
|)
(set-info :category "crafted")
(declare-fun a_12 () (Array Int Int))
(declare-fun a_15 () (Array Int Int))
(declare-fun a_18 () (Array Int Int))
(declare-fun a_20 () (Array Int Int))
(declare-fun a_22 () (Array Int Int))
(declare-fun a_24 () (Array Int Int))
(declare-fun a_27 () (Array Int Int))
(declare-fun a_3 () (Array Int Int))
(declare-fun a_6 () (Array Int Int))
(declare-fun a_9 () (Array Int Int))
(declare-fun e_0 () Int)
(declare-fun e_1 () Int)
(declare-fun e_10 () Int)
(declare-fun e_11 () Int)
(declare-fun e_13 () Int)
(declare-fun e_14 () Int)
(declare-fun e_16 () Int)
(declare-fun e_17 () Int)
(declare-fun e_19 () Int)
(declare-fun e_2 () Int)
(declare-fun e_21 () Int)
(declare-fun e_23 () Int)
(declare-fun e_25 () Int)
(declare-fun e_26 () Int)
(declare-fun e_4 () Int)
(declare-fun e_5 () Int)
(declare-fun e_7 () Int)
(declare-fun e_8 () Int)
(declare-fun a () (Array Int Int))
(declare-fun i () Int)
(assert (= a_12 (store a_9 e_10 e_11)))
(assert (= a_15 (store a_12 e_13 e_14)))
(assert (= a_18 (store a e_10 e_17)))
(assert (= a_20 (store a_18 e_7 e_19)))
(assert (= a_22 (store a_20 e_4 e_21)))
(assert (= a_24 (store a_22 e_0 e_23)))
(assert (= a_27 (store a_24 e_25 e_26)))
(assert (= a_3 (store a e_0 e_2)))
(assert (= a_6 (store a_3 e_4 e_5)))
(assert (= a_9 (store a_6 e_7 e_8)))
(assert (= e_0 (+ i 1)))
(assert (= e_1 (select a i)))
(assert (= e_10 (+ i 4)))
(assert (= e_11 (+ e_1 4)))
(assert (= e_13 (+ i 5)))
(assert (= e_14 (+ e_1 5)))
(assert (= e_16 (select a e_13)))
(assert (= e_17 (- e_16 1)))
(assert (= e_19 (- e_16 2)))
(assert (= e_2 (+ e_1 1)))
(assert (= e_21 (- e_16 3)))
(assert (= e_23 (- e_16 4)))
(assert (= e_25 (+ i 0)))
(assert (= e_26 (- e_16 5)))
(assert (= e_4 (+ i 2)))
(assert (= e_5 (+ e_1 2)))
(assert (= e_7 (+ i 3)))
(assert (= e_8 (+ e_1 3)))
(assert (= a_15 a_27))
(assert (not (= e_14 e_16)))
; --- Injected Array Sum Constraint ---
(assert (sum a_24 (+ e_2 (+ e_11 e_8))))
(assert (sum a_9 (+ e_2 (+ e_11 e_26))))
(assert (sum a_15 (+ e_5 (+ e_4 e_17))))
(assert (sum a_27 (+ e_17 (+ e_5 e_25))))
(assert (sum a_6 (+ e_8 (+ e_4 e_25))))
(assert (sum a_6 (+ e_0 (+ e_23 e_17))))
(assert (sum a_12 (+ e_21 (+ i e_7))))
(assert (sum a_6 (+ e_17 (+ e_10 e_10))))
(assert (sum a_27 (+ e_0 (+ e_13 e_2))))
(assert (sum a_6 (+ e_16 (+ e_19 e_16))))
(check-sat)
(exit)